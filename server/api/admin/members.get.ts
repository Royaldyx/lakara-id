...
export default defineEventHandler(async (event) => {
  ...
  const stores = await query<any>(
    "SELECT id, slug, name, member_email, member_active, active, member_password, products, product_tier, tier_started_at, tier_expires_at, email_verified, created_at FROM stores"
  )
  ...
  const members = stores.map((s: any) => ({
    ...
    email_verified:  s.email_verified === 1 || s.email_verified === true,
