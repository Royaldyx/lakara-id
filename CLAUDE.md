...
**Batch fixes + growth (Juni 2026, pasca-upgrade Nuxt 3.21):**
- **Fix duplicate `hashPassword`** — `portal.ts` tak lagi re-export `hashPassword` (hilangkan warning unimport "Duplicated imports"). 4 importer (`portal/auth/login`, `portal/clients`, `portal/users`, `portal/team` .post) sekarang import `hashPassword` dari `~/server/utils/member`.
- **Article JSON-LD diperkuat** — `image` jadi array + fallback `og-cover.png`, `author` jadi Organization+url, `headline` di-cap 110 char (clear non-critical warning Rich Results).
- **withTransaction di flow kritis** — `tripay/callback.post.ts` (mark paid + upgrade tier kini atomik; referral tetap best-effort setelah commit) & `member/register.post.ts` (insert stores + pending_members atomik). Util `withTransaction` dari `db.ts`.
- **Pro-conversion nudge** — banner upgrade (gradient amber) di `member/dashboard.vue` khusus free tier (atau pro/premium expired), list fitur terkunci → CTA `/member/upgrade`. computed `isFree`.
- **Analytics member lebih dalam** — **MIGRATION WAJIB: `analytics-enrich-migration.sql`** (ALTER `link_clicks` +`referrer` +`device`). Util `server/utils/analytics.ts` (`refSource`, `detectDevice`). `record-click`/`record-view` simpan referrer (dari `document.referrer` yg dikirim `[toko]/index.vue`) + device (dari UA). `analytics.get.ts` +`sources` (top traffic), +`devices` (breakdown), +`hourly` (jam ramai 0-23, dari `HOUR(clicked_at)`). UI `analytics.vue`: 3 section baru (Sumber Kunjungan, Perangkat, Jam Ramai).
- **Template premium** (tanpa migration) — 4 template baru premium-only di `member/links/index.vue` (`aurora`, `luxe`, `glass`, `retrowave`) dgn flag `premium:true`, badge crown, `applyTemplate` blok non-premium. `VALID_TEMPLATES` di `links.post.ts` disinkronkan (+4 key) biar persist.
- **Import produk dari Shopee (best-effort, GRATIS, tanpa migration/dependency)** — `server/api/member/import-product.post.ts`: parse `shopid`/`itemid` dari URL Shopee → fetch API internal `v4/item/get` (header browser, timeout 9dtk) → balikin name/desc/price/price_original/images(URL CDN susercontent)/rating. Graceful fail (Shopee anti-bot / block IP server cPanel = sering gagal, itu wajar). Gambar = hotlink URL Shopee CDN (nggak di-download). UI: kartu "Import cepat dari Shopee" di atas form `member/products/edit.vue` (hanya mode tambah) → prefill form sbg draft, user cek+edit sebelum simpan. **Single product only** (link toko/bulk BELUM — blocking berat). Framing jujur di UI: "kadang gagal, isi manual".
  - **Bookmarklet (jalur ANDAL)** — karena server cPanel hampir selalu diblok Shopee (IP datacenter), jalur utama = bookmarklet yang jalan di browser user di halaman Shopee (same-origin fetch ke `v4/item/get`, pakai sesi user → nggak keblokir), lalu `window.open('lakara.id/member/products/edit?shopee_import=<json>')`. `pages/member/products/edit.vue` baca query `shopee_import` di `onMounted` → `applyImport()`. Kode bookmarklet + cara pasang di **`SHOPEE_IMPORT_BOOKMARKLET.md`**. Card import di member editor DISEMBUNYIKAN (`v-if="false"`) — dipakai admin lewat bookmarklet.
  - **FIX bounce (Juli 2026):** tab baru dari bookmarklet buka `/member/products/edit?shopee_import=` tapi bootstrap auth SPA mem-bounce ke dashboard → query hilang. Solusi robust: `plugins/shopee-import.client.ts` tangkap `?shopee_import` dari `location.search` PALING AWAL → `sessionStorage`. `layouts/member.vue` onMounted: kalau ada `sessionStorage.shopee_import` & bukan di editor → `navigateTo('/member/products/edit')`. `edit.vue` onMounted baca query ATAU sessionStorage (robust parse JSON, single-decode), applyImport, hapus sessionStorage, tampil banner sukses hijau. Jalan walau kebanting ke dashboard dulu.
- **See/hide password** — komponen `components/PasswordInput.vue` (UInput + tombol mata di trailing slot). Diterapkan ke 14 field password di 10 halaman (login admin/member/client, register, account, admin members/stores, portal users/team/clients).
- **QR Code link bio** — kartu di `member/dashboard.vue` (QR via `api.qrserver.com`, zero-dep) + tombol Download (fetch blob → download PNG). Buat promosi offline (banner/merch/booth).
- **Growth badge** — branding "Buat link bio-mu di Lakara" di `[toko]/index.vue` (bio gratis) sekarang link ke `member/register?ref={slug}` → pemilik toko dapat reward referral kalau ada yang daftar. Premium tetap bisa hide branding.
- **Safe-area WebView (Android app)** — app WebView `lakara.id/member` gambar edge-to-edge; Android WebView TIDAK isi `env(safe-area-inset-top)`, app nyuntik CSS var `--safe-area-inset-top`. Fix di `layouts/member.vue`: pakai `max(env(safe-area-inset-top,0px), var(--safe-area-inset-top,0px))` (jalan iOS + Android WebView) di top bar (`.mbr-topbar`), main (`.mbr-main`), sidebar (top+bottom via `safeTop`/`safeBottom`), & bottom nav. `nuxt.config.ts` viewport +`viewport-fit=cover`. **Catatan:** kalau `/admin` atau `/client` juga dibungkus app, terapkan pola sama di `layouts/admin.vue` & `layouts/portal.vue`.
- **Keyboard nutupin input (mobile)** — `plugins/scroll-focus.client.ts`: pas input/textarea di-fokus di layar <1024px, `scrollIntoView({block:'center'})` (delay 300ms) biar field naik di atas keyboard. **Catatan app:** fix penuh butuh `android:windowSoftInputMode="adjustResize"` di activity WebView.
- **PWA offline (tanpa modul, TIDAK pakai @vite-pwa)** — `public/sw.js` di-upgrade (VERSION `lakara-v2`): aset statis (`/_nuxt`, gambar, css/js) cache-first + stale-while-revalidate; navigasi/HTML network-first + fallback cache (app shell tetap kebuka offline); `/api/` network-only (data nggak muncul offline = by design). Registrasi tetap via `plugins/pwa.client.ts`. Zero-dep, zero risiko build. Naikkan VERSION tiap ubah SW.
- **Verify email manual dari admin** — action `verify_email` di `admin/members.post.ts` (set `email_verified`, body `verified:true/false`). `members.get.ts` +`email_verified`. UI `admin/members/index.vue`: badge status + tombol "Verifikasi" per member (buat akun dummy/show tanpa buka DB). Login diblokir sampai verified (`auth.post.ts`) — tombol ini buka blokir.
- **Ikon/logo custom per link (Premium)** — tiap link bio bisa ganti ikon default (brand) pakai upload gambar sendiri. Field baru `link.icon` di `links_bio.links[]`. `links.post.ts` → `sanitizeIcon()` (PREMIUM-only, `/api/file/` atau https). Editor `member/links/index.vue`: section "Ikon / Logo Link" di modal edit (upload+reset, gated premium), `uploadLinkIcon()`/`removeLinkIcon()`. Render `link.icon` di halaman publik `[toko]/index.vue` (link biasa + featured) & preview list editor — fallback ke brand icon kalau kosong. Tanpa migration (JSON). Orphan cleanup aman (collectUploadFiles scan links_bio rekursif). Stock logo belum (butuh aset kurasi) — baru upload.
- **Icon picker + warna icon per link (SEMUA TIER, Juli 2026, tanpa migration)** — di modal Edit Link ada galeri **Tabler brand icon** (44 icon: sosmed, marketplace, generic) + **color picker** (warna bebas + 8 preset). Monokrom → bisa diwarnai. Upload gambar sendiri TETAP Premium. Field baru `link.icon_color` di `links_bio.links[]`; `link.icon` sekarang bisa berupa **nama icon** (`i-tabler-brand-*`) ATAU URL upload. **⚠️ `nuxt.config.ts` `icon.clientBundle.icons` di-isi 44 icon** (WAJIB — dipilih dinamis, harus di-bundle biar tidak fetch runtime & muncul offline). `links.post.ts`: `sanitizeIcon()` sekarang izinkan icon name via whitelist `ALLOWED_ICONS` (semua tier) + upload/https (Premium); `icon_color` divalidasi `isHex`. Render pakai helper `isIconName(v)` (deteksi `i-` prefix, bukan `/api/file/` / http) di `member/links/index.vue` (preview list + modal) & `[toko]/index.vue` (link biasa + featured): kalau icon name → `<UIcon :style="{color}">`, kalau URL → `<img>`, else brand default. `ALLOWED_ICONS` di server HARUS sinkron dgn `iconLibrary` (editor) + `clientBundle.icons` (nuxt.config). Orphan cleanup aman (icon name bukan file).
- **Ukuran/bentuk icon + custom featured link (SEMUA TIER, Juli 2026, tanpa migration)** — 3 field global baru di `links_bio`: `icon_size` (sm/md/lg), `icon_shape` (circle/rounded/square), `icon_hidden` (bool), `social_size` (sm/md/lg). Per-link featured: `feat_bg`/`feat_text` (hex) + `feat_badge` (maks 12 char). `links.post.ts` validasi semua (all-tier; feat_badge di-trim). Editor `member/links/index.vue`: card "Ikon Link" (ukuran/bentuk/toggle sembunyikan) + ukuran social di card Ikon Sosial (`sizeOptions`/`shapeOptions`); custom featured (warna tombol/teks + badge) muncul di modal Edit Link saat toggle Unggulan ON. Publik `[toko]/index.vue`: computed `iconWrapClass`/`iconGlyphClass`/`iconShapeClass`/`iconHidden` + `socialWrapClass`/`socialIconClass` (map sm/md/lg); `featuredStyle` override `background`=feat_bg & `color`=feat_text (non-image); badge pill gantikan bintang kalau `feat_badge` diisi. Field ditambah ke `DEFAULT_BIO` + hydration editor (backward-compat via `{...DEFAULT_BIO, ...lb}`). Featured toggle TETAP Pro (existing), jadi custom featured ikut kegate Pro.
- **Galeri Tema 1-klik + onboarding (Juli 2026, tanpa migration)** — modal galeri tema (`showTemplateGallery`) di `member/links/index.vue`: grid preview besar 16 template, klik → `pickTheme()` apply + `save()` langsung (biar "langsung jadi"). Tombol "Galeri Tema" di card Template (tab Tampilan). **Auto-tawarin ke member baru**: `maybeOfferOnboarding()` di onMounted — kalau `template==='none'` & belum ada link & belum pernah dismiss (localStorage `lakara_onboard_theme_<storeId>`), buka galeri (mode onboarding, ada tombol "Nanti aja"). Reuse `templates`/`applyTemplate`/`templatePreviewBg/Btn`/`getFontStyle` existing.
- **Email digest mingguan (cron, Juli 2026, tanpa migration)** — `server/api/cron/weekly-digest.get.ts` (guard `cronSecret`→fallback `adminPass`, sama pola expire-tiers). Hitung dari `link_clicks`: klik 7 hari + growth vs minggu lalu + views + top link per store (agregat GROUP BY, map di JS). Kirim `sendEmail` HTML cakep (brand gradient + 2 KPI card + top link + CTA analitik) HANYA ke member dengan **view>0** 7 hari terakhir + `member_active=1` + email verified (re-engagement, hindari email 0-klik yg demotivasi). Opsi query: `&dry=1` (hitung saja), `&test=EMAIL` (kirim 1 ke alamat uji), `&limit=N`. **Pasang cron cPanel mingguan** (mis. Senin 08:00): `curl -s "https://lakara.id/api/cron/weekly-digest?key=SECRET"`. Butuh SMTP env terisi.
- **Loading skeleton halaman publik (Juli 2026)** — `pages/[toko]/index.vue`: `useFetch` sekarang expose `pending`; tambah blok `v-else-if="pending"` (skeleton dark: avatar bulat + garis + 4 social + 5 tombol, `animate-pulse`) SEBELUM blok 404. Fix flash blok "tidak ditemukan" saat navigasi SPA sebelum data siap. 404 hanya muncul kalau fetch selesai & data null.
- **FIX KRITIS: artikel hilang saat simpan (Juli 2026)** — `admin/artikel.post.ts` NIMPA seluruh `artikel.json` dengan `body` mentah. Bug: `admin/artikel/edit.vue` load daftar pakai `useFetch(..., { default: () => [] })`; kalau load gagal/belum ready → `allArtikel.value = []` → simpan artikel baru = `[...[], newItem]` = **cuma 1 artikel** → nimpa file → semua artikel lain HILANG (draft `published:false` bikin makin keliatan "hilang semua"). FIX: **(server)** `artikel.post.ts` sekarang wajib `Array.isArray(body)`, bikin **rotating backup** `artikel.json.bak`/`.bak2` sebelum nimpa, + guard 409 tolak nimpa daftar berisi dengan array KOSONG. **(client)** `edit.vue` `save()` ambil daftar TERBARU via `$fetch('/api/admin/artikel')` tepat sebelum simpan & **abort kalau gagal** (jangan pernah merge onto `[]`); merge pakai `current` bukan `allArtikel.value`. Recovery data lama: restore `/home/lakaraid/data/artikel.json` dari backup cPanel/JetBackup (backup `.bak` baru jalan setelah deploy fix ini). Seed 5 artikel ada di `artikel.json` root project.
- **FIX SEO: meta hilang di SSR + www redirect (Juli 2026)** — audit nemu HTML live cuma emit meta default `nuxt.config` (og:title/og:description/canonical dari `useSeoPage` NGGAK ada di SSR). Sebab: `useSeoPage` dulu `await useFetch('/api/seo')` DULU baru `useHead` → kalau fetch lambat/gagal di server, useHead nggak jalan di SSR → Google cuma baca default generik. **FIX:** `composables/useSeoPage.ts` sekarang panggil `useHead` SINKRON (langsung), fetch `/api/seo` dibuat `lazy:true, server:false` (client-only, non-blocking) → SSR SELALU emit meta dari `defaults`. Konsekuensi: **SEO Settings admin untuk halaman statis kini override di CLIENT**, SSR pakai `defaults` — jadi `defaults` per halaman WAJIB diisi bagus (homepage `index.vue` sudah diisi copy target: title "Lakara - Jasa Website, Social Media & Talent Esports Indonesia" + desc + og-cover + canonical). Fallback og image default `https://lakara.id/og-cover.png`. **www→non-www:** `server/middleware/redirect-www.ts` 301 redirect `www.lakara.id`→`lakara.id` (konsolidasi index Google yang sempat kepecah). Catatan: judul/desc di hasil Google bisa tetap di-rewrite Google sendiri (title=site name, desc=cuplikan dari konten mis. FAQ) — itu perilaku Google, bukan bug; minimal sekarang meta asli sudah ada di SSR. Setelah deploy: minta re-index di Google Search Console.
- **SEO hardening batch (Juli 2026, organik/gratis)** — audit + fix menyeluruh biar artikel & halaman kebaca maksimal di Google. **(1) Canonical self-referencing di SEMUA halaman**: `useSeoPage` fallback canonical ke `https://lakara.id{route.path}` (dulu kosong kalau admin tak isi) + `og:url`; `artikel/[slug].vue` canonical `artikel.canonical || pageUrl` (dulu cuma muncul kalau field diisi); `artikel/index.vue` +canonical/og. **(2) robots** di `nuxt.config` → `index, follow, max-image-preview:large, max-snippet:-1, max-video-preview:-1` (rich result eligible). **(3) `<html lang="id">`** via `htmlAttrs`. **(4) Organization JSON-LD** di `app.vue`: `sameAs` dibetulin ke sosmed ASLI (instagram/wearelakara, youtube/@lakaracreative, wa.me) + `contactPoint` + `description` + `alternateName`; WebSite +`inLanguage`. **(5) Article schema** (`artikel/[slug].vue`) +`inLanguage`, `articleSection`, `keywords` + `article:published_time`/`modified_time` meta. **(6) Image sitemap**: `sitemap.xml.ts` artikel kini emit `<image:image>` (namespace image ditambah). Catatan: FAQPage schema sengaja TIDAK dipasang (Google batasi FAQ rich result ke situs otoritatif sejak 2023). Semua zero-dep. **Wajib pasca-deploy: Google Search Console → submit sitemap + Request Indexing per URL artikel** (indexing tetap butuh waktu utk domain baru).
- **App min-version gate (mobile)** — app buka `lakara.id/member?appVersion=x.y.z+build`. `server/api/app-config.get.ts` baca `settings` (`app_min_version`, `app_latest_version`, `app_update_url`; default `0.0.0+0` = izinkan semua). `components/AppUpdateGate.vue` (di-mount di `layouts/member.vue`): tangkap `appVersion` dari query → simpan sessionStorage → fetch config → kalau di bawah min, tampilkan layar "Update Aplikasi" (blocking). Bisa diatur dari **Admin → Settings → Aplikasi Mobile**. Format versi: semver+build (`1.2.0+15`).
