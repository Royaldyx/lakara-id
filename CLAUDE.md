...
- **Safe-area WebView (Android app)** — app WebView `lakara.id/member` gambar edge-to-edge; Android WebView TIDAK isi `env(safe-area-inset-top)`, app nyuntik CSS var `--safe-area-inset-top`. Fix di `layouts/member.vue`: pakai `max(env(safe-area-inset-top,0px), var(--safe-area-inset-top,0px))` (jalan iOS + Android WebView) di top bar (`.mbr-topbar`), main (`.mbr-main`), sidebar, & bottom nav (`safeBottom`). `nuxt.config.ts` viewport +`viewport-fit=cover`. **Catatan:** kalau `/admin` atau `/client` juga dibungkus app, terapkan pola sama di `layouts/admin.vue` & `layouts/portal.vue`.
